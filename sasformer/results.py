...
def top_k_acc(true, pred, k=3):
    n = len(true)
    n_correct = np.sum([np.isin(a, b) for a, b in zip(true, np.argsort(pred, axis=1)[:, ::-1][:, :k])])
    return n_correct / n
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt_path", default=None, type=str, help="Checkpoint path", metavar="ckpt_path")
    parser.add_argument("--batch_size", default=1500, type=int, metavar="batch_size")
    ...
    with open(os.path.join(data_dir, "scales.json")) as f:
        scales = json.load(f)

    device = "cuda:0" if namespace.accelerator == "gpu" else "cpu"
    model = SASPerceiverIOModel.load_from_checkpoint(checkpoint_path=namespace.ckpt_path).to(device)
    print(model.hparams, "\n")
    ...
    trainer = pl.Trainer(gpus=1 if namespace.accelerator == "gpu" else None, deterministic=True)
    ...
    # test targets
    true_batches = [batch for batch in test_dataloader]  # noqa: C416
    y_true_clf = torch.cat([batch[1] for batch in true_batches], dim=0).cpu().numpy()
    y_true_reg = torch.cat([batch[2] for batch in true_batches], dim=0).cpu().numpy()
    y_true_reg = model.target_transformer.inverse_transform(y_true_reg)

    # test predictions
    pred_batches = trainer.predict(model, test_dataloader, ckpt_path=namespace.ckpt_path)
    y_pred_clf = torch.cat([batch[0] for batch in pred_batches], dim=0).cpu().numpy()
    y_pred_reg = torch.cat([batch[1] for batch in pred_batches], dim=0).cpu().numpy()
    ...
    for model_name in model_names:
        ...
        acc = accuracy_score(y_true_clf[model_idx], np.argmax(y_pred_clf[model_idx], axis=1))
        acc = np.round(acc, 3)
        top3_acc = top_k_acc(y_true_clf[model_idx], y_pred_clf[model_idx], k=3)
        top3_acc = np.round(top3_acc, 3)
        precision = precision_score(y_true_clf[model_idx], np.argmax(y_pred_clf[model_idx], axis=1), average="micro")
        precision = np.round(precision, 3)
        recall = recall_score(y_true_clf[model_idx], np.argmax(y_pred_clf[model_idx], axis=1), average="micro")
        ...
        print(
            f"{model_name.ljust(27)}   Acc: {acc:.5f}   "
            f"Top 3 Acc: {top3_acc:.5f}   Precision: {precision:.5f}"
            f"Recall: {recall:.5f}"
        )
    ...
    model_param = [(x[0].split("=")[-1], x[1].split("=")[-1]) for x in model_param]
    ...
    plt.savefig(
        os.path.join(results_dir, "confusion.pdf"),
        bbox_inches="tight",
        transparent=False,
        pad_inches=0,
    )
