...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="../logs/",
        type=str,
        help="Logging directory for Tensorboard/WandB",
        metavar="log_dir",
    )
    parser.add_argument(
        "--subsample",
    ...
    parser.add_argument("--accumulate_grad_batches", default=1, type=int, metavar="accumulate_grad_batches")
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Resume search from checkpoint if it exists.",
        metavar="resume",
    )
    ...
    trials_results_path = os.path.join(root_dir, namespace.log_dir, f"{namespace.project_name}-results.csv")
    study_path = os.path.join(root_dir, namespace.log_dir, f"{namespace.project_name}-study.pkl")
    ...
    study.optimize(
        lambda trial: objective(trial, namespace, root_dir, data_dir),
        n_trials=100,
        gc_after_trial=True,
        callbacks=[
            lambda study, trial: study.trials_dataframe().to_csv(trials_results_path, index=False),
