...
def find_batch_size_one_gpu(params, datamodule):
    ...
    trainer = pl.Trainer(gpus=1, strategy=None, enable_checkpointing=False, deterministic=True, detect_anomaly=False)
    tuner = Tuner(trainer)
    batch_size = tuner.scale_batch_size(model, datamodule=datamodule, mode="binsearch", init_val=32, max_trials=6)
    if batch_size > 2048:
        batch_size = 2048
    clear_cache()
    return batch_size
...
def load_hparams_from_namespace(namespace):
    hparams = {
        "num_latents": namespace.num_latents,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0.25,
        type=float,
        help="Proportion of data to split for validation",
        metavar="val_size",
    )
    parser.add_argument(
        "--log_dir",
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="disable logger for debugging.",
        metavar="disable_logger",
    )
    # encoder args
    parser.add_argument("--num_latents", default=64, type=int, metavar="num_latents")
    parser.add_argument("--latent_dim", default=256, type=int, metavar="latent_dim")
    parser.add_argument("--enc_num_blocks", default=1, type=int, metavar="enc_num_blocks")
    parser.add_argument(
        "--enc_num_self_attn_per_block",
        default=3,
        type=int,
        metavar="encoder_num_self_attn_per_block",
    )
    parser.add_argument("--enc_num_self_attn_heads", default=2, type=int, metavar="encoder_num_self_attn_heads")
    parser.add_argument("--enc_num_cross_attn_heads", default=2, type=int, metavar="enc_num_cross_attn_heads")
    parser.add_argument(
        "--enc_cross_attn_widening_factor",
        default=1,
        type=int,
        metavar="enc_cross_attn_widening_factor",
    )
    parser.add_argument(
        "--enc_self_attn_widening_factor",
        default=1,
        type=int,
        metavar="enc_self_attn_widening_factor",
    )
    parser.add_argument("--enc_dropout", default=0.0, type=float, metavar="enc_dropout")
    parser.add_argument("--enc_attn_dropout", default=0.0, type=float, metavar="enc_attn_dropout")
    # model (clf) decoder args
    parser.add_argument("--model_dec_widening_factor", default=1, type=int, metavar="model_dec_widening_factor")
    parser.add_argument("--model_dec_num_heads", default=5, type=int, metavar="model_decoder_num_heads")
    parser.add_argument("--model_dec_dropout", default=0.0, type=float, metavar="model_dec_dropout")
    ...
    parser.add_argument("--n_bins", default=256, type=int, help="n bins for input discretization.", metavar="n_bins")
    parser.add_argument(
        "--masked",
        default=1,
        type=int,
        help="option to randomly mask I(q) beyond certain q index.",
        metavar="masked",
    )
    parser.add_argument(
        "--mask_proportion",
        default=0.23,
        type=int,
        help="proportion of I(q) masked.",
        metavar="mask_proportion",
    )
    parser.add_argument("--clf_weight", default=1.0, type=float, metavar="clf_weight")
    parser.add_argument("--reg_weight", default=1.0, type=float, metavar="reg_weight")
    parser.add_argument("--reg_obj", default="mse", type=str, metavar="reg_obj")
    # lightning trainer args
    parser.add_argument("--batch_size", default=2048, type=int, metavar="batch_size")
    parser.add_argument("--batch_size_auto", default=0, type=int, metavar="batch_size_auto")
    ...
    parser.add_argument("--weight_decay", default=1e-7, type=float, metavar="weight_decay")
    parser.add_argument("--max_epochs", default=100, type=int, metavar="max_epochs")
    parser.add_argument("--gradient_clip_val", default=1.0, type=float, metavar="gradient_clip_val")
    parser.add_argument(
        "--ckpt_path",
        default=None,
        type=str,
    ...
    parser.add_argument("--accumulate_grad_batches", default=None, type=int, metavar="accumulate_grad_batches")
    parser.add_argument("--overfit_batches", default=0, type=int, metavar="overfit_batches")
    parser.add_argument("--detect_anomaly", default=1, type=int, metavar="detect_anomaly")
    parser.add_argument("--profile", default=0, type=int, metavar="profile")
    parser.add_argument("--deterministic", default=1, type=int, metavar="deterministic")
    parser.add_argument(
        "--strategy",
        default=None,
        type=str,
        help="Set to `ddp` for cluster training",
        metavar="strategy",
    )
    parser.add_argument(
        "--num_nodes",
        default=1,
        type=int,
        help="N nodes for distributed training.",
        metavar="num_nodes",
    )
    parser.add_argument("--seed", default=None, type=int, help="Random seed.", metavar="seed")
    ...
    trainer.fit(model, datamodule=datamodule, ckpt_path=namespace.ckpt_path)
